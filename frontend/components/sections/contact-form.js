...
const ContactForm = ({ data }) => {
    ...
    const localeName = useMemo(
        () => (Cookies.get("NEXT_LOCALE") === "en" ? "Name" : "名字"),
        [Cookies]
    )
    const localeEmail = useMemo(
        () => (Cookies.get("NEXT_LOCALE") === "en" ? "Email" : "邮箱"),
        [Cookies]
    )
    const localeMessage = useMemo(
        () => (Cookies.get("NEXT_LOCALE") === "en" ? "Message" : "信息"),
        [Cookies]
    )
    ...
    return (
        ...
        >
            <div className="w-full overflow-hidden md:w-1/3 md:pr-8 pt-4 mb-10 md:mb-0">
                <p className="uppercase text-xs font-medium tracking-widestx2 mb-10 ">
                    {data.title}
                </p>
                ...
                <NoSSR>
                    ...
                    >
                        {({ errors, touched, isSubmitting }) => {
                            if (Cookies.get("NEXT_LOCALE") === "zh-CN") {
                                errors.name = "请输入名字";
                                errors.email = "请输入邮箱";
                            }
                            return (
                                <div>
                                    <Form className="flex flex-wrap">
                                        ...
                                        {success && Cookies.get("NEXT_LOCALE") === "en" && (
                                            <p className="text-base mt-10 ml-10 feedback">
                                                Thanks for your feedback
                                            </p>
                                        )}
                                        {success && Cookies.get("NEXT_LOCALE") === "zh-CN" && (
                                            <p className="text-base mt-10 ml-10 feedback">
                                                感谢您的反馈
                                            </p>
                                        )}
